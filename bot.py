...
async def unmute(ctx, member: discord.Member):
    ...
    # Annule le tempmute si existant
...
async def tempmute(ctx, member: discord.Member, duration: str, *, reason="Aucune raison fournie"):
    ...
    Utilisation : +tempmute @membre/ID durée raison
    Durées : 5min, 2h, 1j, 30s
    """
    ...
    # Annule un ancien tempmute si existant
    if member.id in tempmute_tasks:
        tempmute_tasks[member.id].cancel()
...
async def warnings(ctx, member: discord.Member):
    ...
    await ctx.send(embed=info_embed(
        "⚠️ Avertissements",
        f"{member.mention} a **{count}** avertissement(s)."
    ))
...
async def purge(ctx, amount: int):
    """Supprime X messages dans le salon."""
...
async def ticket_setup(ctx, category: discord.CategoryChannel = None, support_role: discord.Role = None):
    """Configure et crée le panel de tickets."""
    if category:
...
async def automod_toggle(ctx, feature: str, state: str):
    """
    Active/désactive l'auto-modération.
    Features : antispam | antilink | all
    State : on | off
    """
...
async def antilink_bypass(ctx, role: discord.Role):
    """Définit un rôle qui peut envoyer des liens."""
